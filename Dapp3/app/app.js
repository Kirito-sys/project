...
var contractERC721;
function getErc721() {
	contractERC721 = new web3.eth.Contract(contractERC712Abi, "0xf702AAf5DB10764A6B7059cF8FB4A3A698809010");
	console.log('contractERC721==>', contractERC721);
}
var contract = new web3.eth.Contract(cabi, "0x0EFe5EcaEE3E91A8092267DE46063CAaf03251Eb");
...
function addaccount(){
	...
	let _game=document.getElementById('game').value;
	let _price=document.getElementById('price').value;
	let _pass=document.getElementById('pass').value;
	contract.methods.addaccount(tokenURl,_name,_id,_game,_price,_pass).send({from:accounts[0]}).then(
	
			        function (result) {
			            console.log("result:",result);
			            $('.addaccount').html(result.status);
			        }
...
function getaccount(){
    ...
    contract.methods.getAccountText(_tokenId).call({from:accounts[0]}).then(
		        function (result) {
		            console.log("result:",result);
		            $('.getAccountText').html("<p>" + "游戏名:" +result[0] + "" + "账户名:" +result[1] + "" + "价格:" + result[2]);
		        }
		    )
}
async function co(){
	contract.methods.getonwer().call({ from: accounts[0] }).then(
		function (result) {	
			if( result.toUpperCase() == accounts[0].toUpperCase()){
				console.log("您是合约主人")                                                                                  //<button id="Btn_send' + i + '"  onClick="sendBtn(' + i + ')">转账</button>           
...
$("#Btn_CreatItem1").click(function () {
	...
	if (_url.length < 3) {
		...
		contract.methods.addaccount(_url, _Name, _ID).send({ from: accounts[0] }).then(
			function (result) {
				console.log("result", result);
				if (result.status) {
					$(".txt_CreatItem").html("添加成功")
...
$("#buy").click(function (from,TOKENID) {
	...
	console.log("ssssssaaaaaa",TOKENID)
	 from = $("#from").val()
	console.log("ffff",)
    contract.methods.buyaccount(from,TOKENID).send({ from: accounts[0],value: 2 * Math.pow(10, 18) }).then(
        function (result) {
            console.log(result)    
        }
    );
